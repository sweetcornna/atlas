...
| 本仓库许可 | **双许可**（章程 v2.16，2026-08-29）：阡陌自有代码 **AGPL-3.0-or-later**（根 `LICENSE`），基座导入的代码 **MIT**（根 `LICENSE.base`，上游原文逐字保留）。**权威判据是文件在不在基座快照 `base-snapshot/v2.46.0` 里**（`git cat-file -e base-snapshot/v2.46.0:<路径>`），快照之外即阡陌自有。文件头是这条判据的**标记**、不是判据本身：**带头 ⇒ 阡陌自有**成立，**反向不成立**——仓库里有 86 个无头却是阡陌自有的文件。新增阡陌源文件必须带 `Copyright 2026 Qianmo AgentNest Team` 与 `SPDX-License-Identifier: AGPL-3.0-or-later` 这两行头（章程 §5.5）；补头是这条约定本身，归属判据仍是快照——缺头的风险是下游许可扫描器等外部工具默认按文件头判定，会被误判为基座层。范围与理由见 `NOTICE` 一、许可 |
| 阡陌自有代码 | `packages/` 下 `@qianmo/*` workspace 包 + 对基座运行时的常驻化改造 |
| 成果边界 | 基座快照导入于提交 `3380c88`，其后全部提交即阡陌工作（另加导入前的立项提交 `67f6081`、`74f7a22`），唯一例外是纯上游同步提交 `d04a79dd`（2026-08-17，内容全部来自上游补丁）。**2026-08-17 首次上游同步后，工作面用 `git diff base-snapshot/v2.46.0..HEAD` 举证，不要再用 `git diff 3380c88..HEAD`**——后者已含上游 v2.46.0 的 6 万行。**`848ad8c2` / `1128a391` 都是上游 SHA，本仓库不含上游历史，它们在干净 clone 上不是有效 git 对象** |

**本仓库不做的两件事**（章程 N-13/N-14）：

- **不发布 npm 包、不打 tag、不跑基座的 release 流程。**`publish-npm.yml` **已按 roadmap P0.4 移除**（2026-08-25 复核：`.github/workflows/` 下只剩 `ci.yml`）；`scripts/release.ts`、`scripts/releaseCore.ts` 与 `scripts/changelog-section.ts` **已于 2026-08-26 删除**（它们只服务那个已不存在的工作流，留着会让人以为这里有一条发布路径）。**下半部分基座原文的「发布」一节描述的是那个已不存在的 workflow**（`publish-npm.yml` 的门禁链、`ci.yml`/`publish-npm.yml`/`release` 三处共用分片脚本等），读到时按本条为准；那一节是基座逐字原文，**不要去改它**，基座约定的更新走上游同步。看到 `scripts/release.ts`、`CHANGELOG.md` 发布约定、npm 包名同步清单这类基座内容——**那是基座的发布面，不是我们的**，不要去维护它，也不要因为它"看起来该更新"而改它。
...
**基座自己的 `CLAUDE.md` 是架构与工程约定的真源，其全部条款在本仓库继续有效。**本文件**上半部分不复制**它，只做两件事：声明它继续适用，并复述其中与阡陌开发最容易撞上的几条硬规则；基座原文本身逐字附在**本文件下半部分**。

> 基座原文就在**本文件下半部分**（分界注释以下，与上游逐字一致）。**取原文就往下读**——本仓库不含上游历史，`git show 848ad8c2:CLAUDE.md` 跑不通。原先另置的根目录 `BASE-CLAUDE.md` 已于 P10.3① 并回本文件并删除（放置方案就此定案，P0.2 已闭，见 roadmap v2.41）。**上半部分（阡陌部分）是摘要，不是替代**——判断细节以下半部分的基座原文为准。改阡陌约定改上半部分，改基座约定等上游同步。
...
**⑧ 「指针不复制」铁律**：同一个事实只允许存在于一个地方，其他地方只放指针。本文件的阡陌部分本身就在遵守它——所以它是摘要而不是副本（下半部分的基座原文是本仓库内**唯一**一份，不是第二份副本）。

**⑨ 版权头门禁。**基座快照之外的阡陌自有文件必须在前 5 行内按自身注释语法（`//` / `#` / `<!-- -->`）带上那两行头，shebang 文件排在 shebang 之后；基座文件一律不加头，即便改造过也不例外，文件头只是来源层标记，不是归属判据。`bun run check:license-headers` 是双向硬零、无预算，已进 precheck / verify / CI。

最容易踩的是新增仓库里没出现过的格式默认也要带头——这是 fail-closed 的有意设计；要么补头，要么进脚本豁免表并在 `NOTICE` 一、许可说明理由。具体判据与豁免表见 `scripts/check-license-headers.ts` 和 `NOTICE` 一、许可。
...
阡陌的成果边界靠 git 历史证明：基座零改动快照导入于提交 `3380c88`，其后的全部提交即本项目工作，唯一例外是纯上游同步提交 `d04a79dd`（2026-08-17，内容全部来自上游补丁）。**首次上游同步（2026-08-17）之后，度量基线与历史起点分成了两个**——历史起点仍是 `3380c88`，而「基座内容有多少」的度量基线是标签 `base-snapshot/v2.46.0`（无父提交的基座零改动快照）。因此**以下操作一律禁止**：

- rebase 掉导入提交 `3380c88`
- squash 跨越导入提交
- 强推改写导入之后的历史
- 删除或移动 `base-snapshot/*` 标签，或把上游同步提交与阡陌改动 squash 在一起（那会让「哪些行来自上游」无法分离）
...
<!-- ======== 以下为基座 open-claude-code 的 CLAUDE.md 原文（导入点 3380c88 的逐字副本）========
     P10.3①：这一段必须与上游逐字一致，上游同步的补丁才能干净落下（见 docs/dev/upstream-sync-drill.md §5②）。
     改阡陌约定改上半部分；基座约定不要在这里就地改写，等上游同步带进来。
     ============================================================================================ -->
