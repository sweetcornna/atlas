...
- **`docs/dev/charter.md`（章程 v2.0）是 M0 阶段的唯一范围依据。**凡章程 §3 未列入、或 §2.2 非目标已列出的事项，M0 内一律不做。写代码前先确认它在范围内。
...
**优先走扩展点，能不改核心就不改。**基座提供了多个扩展点：daemon worker 注册表、MCP、hooks、workspace 包。走扩展点的改动在将来同步上游时几乎不冲突；改核心的改动每次同步都要重解一遍。
