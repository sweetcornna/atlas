...
> 基座原文保留为根目录 [`BASE-CLAUDE.md`](./BASE-CLAUDE.md)（与 `BASE.md` 配对）。**取原文就读这个文件**——本仓库不含上游历史，`git show 848ad8c2:CLAUDE.md` 跑不通。该放置方案为立项时的落地决定，S0（P0.2）评审可调整。**本文件是摘要，不是替代**——判断细节以 `BASE-CLAUDE.md` 为准。
