...
/**
 * The env var that selects the identity, named once so the places that must
 * SET it — `dist/cli-qianmo.js`, generated by scripts/entrypoints.ts — cannot
 * drift from the place that reads it.
 */
export const IDENTITY_ENV_VAR = 'OCC_IDENTITY'

/** The identity occ presents as unless told otherwise. */
const DEFAULT_IDENTITY_MODE = 'occ'

/**
 * The identity a Qianmo node runs as.
 *
 * Exported because the node's own bin entry pins it: `qm` is not a rename of
 * `occ`, it is a different product surface, so its generated entrypoint sets
 * `IDENTITY_ENV_VAR` to this before anything else loads. Taking the value from
 * here rather than writing `'qianmo'` into the build script keeps the roster
 * below the only place identities are spelled.
 */
export const NODE_IDENTITY_MODE = 'qianmo'
 ...
 */
export const ALL_IDENTITY_MODES = [
...
export const IDENTITY_MODE: IdentityMode =
  ALL_IDENTITY_MODES.find(mode => mode === process.env[IDENTITY_ENV_VAR]) ??
  DEFAULT_IDENTITY_MODE
