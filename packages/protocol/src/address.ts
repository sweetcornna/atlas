import { ProtocolError, ProtocolErrorCode, issue } from "./errors.ts";
...
export function formatAddress(address: QianmoAddress): string {
  if (!isValidSegment(address.node)) {
    throw new ProtocolError([
      issue(ProtocolErrorCode.E_BAD_ADDRESS, "node", `invalid node segment: ${String(address.node)}`),
    ]);
  }
  if (!isValidSegment(address.agent)) {
    throw new ProtocolError([
      issue(ProtocolErrorCode.E_BAD_ADDRESS, "agent", `invalid agent segment: ${String(address.agent)}`),
    ]);
  }
...
export function assertAddress(raw: unknown, field = "address"): QianmoAddress {
  ...
  if (parsed === null) {
    throw new ProtocolError([
      issue(ProtocolErrorCode.E_BAD_ADDRESS, field, `not a ${ADDRESS_SCHEME} address: ${String(raw)}`),
    ]);
  }
  return parsed;
}
