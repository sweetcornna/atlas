 ...
 */
export function createMessage<P>(input: CreateMessageInput<P>): QianmoMessage<P> {
  return {
    v: ENVELOPE_VERSION,
 ...
 */
export function withHop<P>(message: QianmoMessage<P>, node: string): QianmoMessage<P> {
...
export function isExpired(message: QianmoMessage, now: number = Date.now()): boolean {
