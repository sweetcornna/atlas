import { isValidAddress, isValidSegment } from "./address.ts";
import { ProtocolError, ProtocolErrorCode, issue, type ProtocolIssue } from "./errors.ts";
...
export function validateMessage(
  ...
  if (typeof input !== "object" || input === null || Array.isArray(input)) {
    return {
      ok: false,
      issues: [issue(ProtocolErrorCode.E_BAD_ENVELOPE, "", "message must be a plain object")],
    };
  }
  ...
  if (!isNonEmptyString(raw["msgId"])) {
    issues.push(issue(ProtocolErrorCode.E_BAD_ENVELOPE, "msgId", "msgId must be a non-empty string"));
  }
  if (!isNonEmptyString(raw["traceId"])) {
    issues.push(
      issue(ProtocolErrorCode.E_BAD_ENVELOPE, "traceId", "traceId must be a non-empty string"),
    );
  }
  if (!isValidAddress(raw["from"])) {
    issues.push(
      issue(ProtocolErrorCode.E_BAD_ADDRESS, "from", `invalid sender address: ${String(raw["from"])}`),
    );
  }
  if (!isValidAddress(raw["to"])) {
    issues.push(
      issue(ProtocolErrorCode.E_BAD_ADDRESS, "to", `invalid recipient address: ${String(raw["to"])}`),
    );
  }
  if (!isMessageType(raw["type"])) {
    issues.push(issue(ProtocolErrorCode.E_BAD_TYPE, "type", `unknown message type: ${String(raw["type"])}`));
  }
  if (!("payload" in raw)) {
    issues.push(issue(ProtocolErrorCode.E_BAD_ENVELOPE, "payload", "payload field is required"));
  }
  if (!isPositiveFinite(raw["createdAt"])) {
    issues.push(
      issue(ProtocolErrorCode.E_BAD_ENVELOPE, "createdAt", "createdAt must be a positive epoch time"),
    );
  }
  if (!isPositiveFinite(raw["ttlMs"])) {
    issues.push(issue(ProtocolErrorCode.E_BAD_ENVELOPE, "ttlMs", "ttlMs must be a positive number"));
  }

  const hops: unknown = raw["hops"];
  if (!Array.isArray(hops)) {
    issues.push(issue(ProtocolErrorCode.E_BAD_ENVELOPE, "hops", "hops must be an array of node names"));
  } else {
    const hopList: readonly unknown[] = hops;
    if (!hopList.every(isValidSegment)) {
      issues.push(issue(ProtocolErrorCode.E_BAD_ADDRESS, "hops", "hops must contain valid node names"));
    }
  }
  ...
  if (new Set(message.hops).size !== message.hops.length) {
    issues.push(issue(ProtocolErrorCode.E_LOOP, "hops", "duplicate node in hops"));
  }
  if (options.node !== undefined && message.hops.includes(options.node)) {
    issues.push(
      issue(ProtocolErrorCode.E_LOOP, "hops", `message already visited node ${options.node}`),
    );
  }
  ...
  if (bytes > maxBytes) {
    issues.push(
      issue(ProtocolErrorCode.E_TOO_LARGE, "", `message is ${bytes} bytes, limit is ${maxBytes}`),
    );
  }
...
export function firstErrorCode(result: ValidationResult): ProtocolErrorCode | null {
