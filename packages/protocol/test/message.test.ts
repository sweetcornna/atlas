...
describe("createMessage", () => {
  ...
  test("destinationNode reads the recipient node", () => {
    const message = createMessage({ from: FROM, to: TO, type: MessageType.Ping, payload: 1 });
...
describe("withHop", () => {
  ...
  test("throws E_TOO_MANY_HOPS past the limit", () => {
    ...
    try {
      ...
      expect((error as ProtocolError).code).toBe(ProtocolErrorCode.E_TOO_MANY_HOPS);
    }
  });
});
...
describe("ttl and size", () => {
  ...
  test("messageBytes measures the utf-8 serialization", () => {
    expect(messageBytes(message)).toBe(new TextEncoder().encode(serializeMessage(message)).length);
...
describe("errorReply", () => {
  test("addresses the reply back to the sender and keeps the trace", () => {
    ...
    const reply = errorReply(original, ProtocolErrorCode.E_UNKNOWN_AGENT, "no such agent", 5_000);
