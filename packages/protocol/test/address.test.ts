...
describe("address", () => {
  ...
  test("formatAddress throws E_BAD_ADDRESS on an invalid segment", () => {
    expect(() => formatAddress({ node: "Bad Node", agent: "planner" })).toThrow(ProtocolError);
    try {
      formatAddress({ node: "ok", agent: "" });
      throw new Error("expected formatAddress to throw");
    } catch (error) {
      expect(error).toBeInstanceOf(ProtocolError);
      expect((error as ProtocolError).code).toBe(ProtocolErrorCode.E_BAD_ADDRESS);
  ...
  test("addressEquals compares structurally", () => {
    expect(addressEquals({ node: "n", agent: "a" }, { node: "n", agent: "a" })).toBe(true);
    expect(addressEquals({ node: "n", agent: "a" }, { node: "n", agent: "b" })).toBe(false);
  });
