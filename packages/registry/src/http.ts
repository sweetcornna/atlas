...
function methodNotAllowed(allowed: readonly string[]): Response {
  return fail(405, RegistryErrorCode.E_BAD_REQUEST, `allowed methods: ${allowed.join(", ")}`, {
    allow: allowed.join(", "),
  });
}
...
async function handleCollection(
  ...
  if (body === null) {
    return fail(400, RegistryErrorCode.E_BAD_REQUEST, "body must be a JSON object");
  }
...
function handleItem(request: Request, registry: InMemoryRegistry, name: string): Response {
  if (request.method === "GET") {
    const entry = registry.resolve(name);
    return entry === null ? notFound(`no live agent named ${name}`) : json(agentBody(entry));
  }
