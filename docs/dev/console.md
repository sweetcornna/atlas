...
对控制台而言，这两条恰好就是上面第 2 步与第 3 步：**没有第 2 步，第 3 步只是让唤醒不被拒，
不会让它被执行。**两件事的后果不同，排障时要分开看：
...
一条没签名的唤醒在开放策略下会被送达，但没有任何东西为它背书，所以它是 `untrusted`，agent
不会去执行。这正是现网四台在走完上面三步之前的状态——**是对的，不要为了让现网好看而放宽。**
