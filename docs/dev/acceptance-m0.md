...
§4.1 记录 AC-6(a) 仍有一个开放的提请评审问题（沙箱是否直接采用基座权限模式作第一道、容器作第二道），**该问题在文档中未见闭合记录**。
...
- §4.1 的 AC-6(a) 提请评审问题**未见闭合记录**——走查时若被问到，如实说「留在 S0 评审、未见书面结论」。
...
### 4.3 AC-1 判据②：证据是叙述，不是留档

- **事实**：「不重放历史即可续答」的唯一记录是 roadmap 速查表 P1.2 行与 v2.13 的**文字叙述**（代号 `QM-seven-bridges`、端到端 5.2 s）。**仓库内没有报告文件、没有 SHA 锚定**，`demo/ac1-restart.sh` 里这一段是**恒 SKIP**。
- **性质**：判据本身达标（有人做过并记录了），但**证据形态弱于其他七条**。
- **处置建议**：走查现场重做一次并留档即可关闭——成本只是一份凭据加十分钟。**这条不需要豁免，需要的是一次现场演示。**
...
| provider fixture | `cat tests/integration/fixtures/qianmo-providers.json` | 确为 **2 条**（`qianmo-deepseek` / `qianmo-qwen`），`kind` 均为 `openai-compat`，`baseUrl` **相同**（G-4 的直接证据） |
