...
> **待 P13.2**：本表将新增 `notify` 一行（agent → 已建立通道对端的主动通知），并在上一段后补充 `notify` 与本段否决的 `task.progress` 的区分理由——两者容易被误认为同一件事，实则 `notify` 不产生 ack、不进任务状态机、不复用 `taskId`，三条都不在 `task.progress` 被否决的理由之内。详见 `docs/dev/resident-botization.md` §2.8、本文 §14（占位）。**本表现有内容一字未改。**
...
> **待 P13.2**：本节将补一句规则——非回复类的新消息类型必须自带 fresh `taskId`，不得复用引发它的任务的 `taskId`（否则同一上下文的第二条该类消息会在判环表上命中并被误判 `E_LOOP`）。`notify` 是该规则的首个适用对象，例子见 `docs/dev/resident-botization.md` §2.4②。**本节现有内容一字未改。**
...
> **待 P13.2**：新消息类型 `notify` 将补入本节，与 `ping`/`pong` 同列——不进下方的任务状态机，只有独立的 `created → sent → delivered/expired` 三态。详见 `docs/dev/resident-botization.md` §2.8。**本节现有内容一字未改。**
...
> **待 P13.2**：本表将新增 `E_BUSY`（节点不接新活：队列满或 ESTOP 已启）；同时新增一条兼容规则——新错误码不是自由加法，只能发给已在能力发现里声明支持它的对端，对未声明的对端一律降级为语义最近的既有码（`E_BUSY` 降级为 `E_RATE_LIMITED`）。理由与旧节点兼容陷阱见 `docs/dev/resident-botization.md` §2.6。**本表现有内容一字未改。**
...
## 14. 主动通知（notify）——待 P13.2 落地

**占位小节，本节暂不写正文。** `notify` 是 M1 P13.2 引入的新消息类型：处理方节点 → 已建立通道的对端（M1 内实际只有中枢一个消费者）的主动通知，**不进任务状态机、不产生协议 ack、不复用任何既有 `taskId`**。完整设计（payload 形状、判环归组、去重、TTL、回执语义、限流、能力发现降级）见 `docs/dev/resident-botization.md` §2。

本文将在 P13.2 代码落地的同一个 PR 内补齐本节正文，并同步落地上文四处已标注「待 P13.2」的改动点（§3.4 消息类型表、§6.1 判环规则、§8 状态机、§11 错误码表）。**在正文落地前，本节与上述四处标注均不构成协议承诺**，`packages/protocol` 的实际枚举与校验逻辑以代码为准。
