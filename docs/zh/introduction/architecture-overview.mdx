...
| 入口 | 文件 | 说明 |
|------|------|------|
| CLI 启动 | `src/entrypoints/cli.tsx` | 从 `bun:bundle` 导入 `feature()`（取值由构建 / dev defines 在编译期求值）。**MACRO 全局变量没有运行时兜底**：`MACRO.*` 是转译期替换，直跑源码的进程必须自己用 `-d` 注 defines（`scripts/defines.ts` 的 `macroDefineArgs()`），缺了就在第一次读时抛错 |
