...
真正的代码入口是 `src/entrypoints/cli.tsx`，它做了两件关键的事：
...
**这里没有 MACRO 全局变量的运行时兜底，是刻意的。**`MACRO.*` 是转译期替换，
直跑源码的进程必须自己注 defines（`scripts/defines.ts` 的 `macroDefineArgs()`）；
缺了就在第一次读时抛错，而不是拿到一份手抄的、会静默漂移的值。
