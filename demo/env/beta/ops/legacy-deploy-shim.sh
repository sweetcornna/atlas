...
# 这个脚本已被仓库里的 demo/env/beta/beta-deploy.sh 取代（2026-08-26）。
#
# 为什么换掉：原来的版本每次部署都留一份备份、**从不清理**。2026-08-26 一天四次
# 部署之后 workbench-iap 累到 8 份约 13 GB，30 GB 的盘 100% 满 —— `mv` 已成功、
# `cp` 中途没空间，控制台与注册中心一起下线，只剩一棵 589 MB 的半截树。
# 原件留在同目录的 *.superseded-20260826，仅供查阅，别再跑它。
#
...
它不起进程。树上有进程跑着会**拒绝**换 —— 先 demo/env/beta/beta-down.sh <节点> 停掉，
换完再 beta-up.sh 起回来。注意起机时要把停机前 argv 里 `--` 之后的透传参数原样带回去
